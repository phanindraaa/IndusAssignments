...
public class AddressDAOTest {
    ...
    public void testGetEmployeesByAddressId() {
        ...
        // Check sorting order
        Iterator<Entry<Long, Employee>> iterator = employees.entrySet().iterator();
        
        Entry<Long, Employee> firstEntry = iterator.next();
        Entry<Long, Employee> secondEntry = iterator.next();
        
        assertTrue("Employees should be sorted by last name", 
                   firstEntry.getValue().getLastName().compareTo(secondEntry.getValue().getLastName()) <= 0);
    }
