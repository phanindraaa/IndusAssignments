...
public class AddressDao {
    ...
    public Address getAddressById(Long addressId) {
        ...
        try {
            ...
            if (address != null) {
                // Accessing the lazy collection while the session is still open
                address.getEmployees().size(); // Initialize the collection
            }
            return address;
        } finally {
            if (session != null) {
    ...
    public Set<Employee> getEmployeesByAddressId(Long addressId) {
        Session session = null;
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            Address address = (Address) session.get(Address.class, addressId);
